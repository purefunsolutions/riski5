...
{- | System clock domain — 40 MHz, period 25 ns. On the DE2 this is
produced by an Altera ALTPLL instantiated in @riski5_top.v@
(generated by @pkgs/riski5-core/package.nix@) from the on-board
50 MHz oscillator at @50 × 4 / 5 = 40 MHz@. Slow-85 °C STA
reports 54.89 MHz Fmax with +1.78 ns slack — comfortable for the
current 5-stage + async-regfile + async-dmem shape. Phase 2B / 2C
will shorten the X cone (M4K regfile, sync dmem + D\$) and lift
the ceiling past 60 MHz.
...
guarantee that is to share one source of truth for @clkSys@ at the
Verilog-wrapper level. Reset is held asserted until the PLL locks
and KEY[0] is released.

The next planned subdivision (task #141) splits the SDRAM
controller IP onto a separate slower domain (e.g. @DomSdram@)
with an async-FIFO Avalon-MM bridge, so back-to-back
ACTIVATE/PRECHARGE/ACTIVATE patterns from the core stop hammering
the IP at the same rate the core retires instructions. Every
@DomSys@ signal in this file stays in the core/peripheral domain;
SDRAM-IP-domain signals will get their own type when that lands.
-}
createDomain
  vSystem
    { vName = "DomSys"
...
-}
topEntity ::
  {- | 40 MHz system clock generated by an Altera ALTPLL sitting in
  the @riski5_top.v@ wrapper outside the Clash module.
  -}
  "CLOCK_SYS" ::: Clock DomSys ->
  {- | Active-low reset, held asserted until the PLL locks and
  @KEY[0]@ is released.
  -}
  "RESET_SYS_N" ::: Reset DomSys ->
  "KEY" ::: Signal DomSys (BitVector 4) ->
  "SW" ::: Signal DomSys (BitVector 18) ->
  ...
  -}
  "SRAM_DQ_I" ::: Signal DomSys (BitVector 16) ->
  ...
  -}
  "UART_RDATA" ::: Signal DomSys (BitVector 32) ->
  ...
  trigger event.
  -}
  "DEBUG_RESET_CAPTURE" ::: Signal DomSys Bool ->
  ...
  -}
  "DEBUG_CAPTURE_OFFSET" ::: Signal DomSys (Unsigned 2) ->
  ...
  -}
  "JTAG_LOAD_MODE" ::: Signal DomSys Bool ->
  -- | L-3 JTAG-load: byte address for the next SDRAM transaction
  --   (source @JLAD@). Ignored when @JTAG_LOAD_MODE = False@.
  "JTAG_LOAD_ADDR" ::: Signal DomSys (BitVector 32) ->
  -- | L-3 JTAG-load: 32-bit write data (source @JLDW@).
  "JTAG_LOAD_WDATA" ::: Signal DomSys (BitVector 32) ->
  -- | L-3 JTAG-load: pulse-high to commit a write (source
  --   @JLWE@); the Tcl script writes 1 then 0.
  "JTAG_LOAD_WE" ::: Signal DomSys Bool ->
  -- | L-3 JTAG-load: pulse-high to issue a read (source @JLRD@).
  --   The result lands on @JTAG_LOAD_RDATA@ once
  --   @JTAG_LOAD_BUSY@ deasserts.
  "JTAG_LOAD_RD" ::: Signal DomSys Bool ->
  ""
    ::: ( "LEDR" ::: Signal DomSys (BitVector 18)
        , "LEDG" ::: Signal DomSys (BitVector 9)
        , "LCD_DATA" ::: Signal DomSys (BitVector 8)
        , "LCD_RS" ::: Signal DomSys Bit
        , "LCD_RW" ::: Signal DomSys Bit
        , "LCD_EN" ::: Signal DomSys Bit
        , "LCD_ON" ::: Signal DomSys Bit
        , "LCD_BLON" ::: Signal DomSys Bit
        , "SRAM_ADDR" ::: Signal DomSys (BitVector 18)
        , "SRAM_DQ_O" ::: Signal DomSys (BitVector 16)
        , "SRAM_DQ_OE" ::: Signal DomSys Bool
        , "SRAM_CE_N" ::: Signal DomSys Bit
        , "SRAM_OE_N" ::: Signal DomSys Bit
        , "SRAM_WE_N" ::: Signal DomSys Bit
        , "SRAM_UB_N" ::: Signal DomSys Bit
        , "SRAM_LB_N" ::: Signal DomSys Bit
        , -- UART bus tap — routed in @riski5_top.v@ to the Altera
          ...
          -- on the same (DomSys) cycle.
          "UART_SEL" ::: Signal DomSys Bool
        , "UART_ADDR" ::: Signal DomSys (BitVector 32)
        , "UART_WDATA" ::: Signal DomSys (BitVector 32)
        , "UART_BE" ::: Signal DomSys (BitVector 4)
        , "UART_RE" ::: Signal DomSys Bool
        , -- SDRAM bus tap — 16-bit Avalon-MM master signals produced
          ...
          "SDRAM_CS" ::: Signal DomSys Bool
        , "SDRAM_ADDR" ::: Signal DomSys (BitVector 22)
        , "SDRAM_WDATA" ::: Signal DomSys (BitVector 16)
        , "SDRAM_BE" ::: Signal DomSys (BitVector 2)
        , "SDRAM_RD" ::: Signal DomSys Bool
        , "SDRAM_WR" ::: Signal DomSys Bool
...
topEntity
  ...
  jtagLoadWeS
  jtagLoadRdS =
  withClockResetEnable clkSys rstSys enableGen
    $ let sdramReplyS =
