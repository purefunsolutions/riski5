...
{- | 50 MHz core domain. The @Dom30@ name is a historical artefact
from the phase-1 30 MHz configuration; the PLL now runs the core
at 50 MHz (phase 2 P2-A). On the DE2 this is produced by an
Altera ALTPLL instantiated in @riski5_top.v@ (generated by
@pkgs/riski5-core/package.nix@) from the on-board 50 MHz
oscillator: @50 × 10 / 10 = 50 MHz@ (i.e. the PLL exists purely
for reset gating + the shared @clk30@ rail, VCO 500 MHz passing
straight through). Slow-85 °C STA reports 54.89 MHz Fmax with
+1.78 ns slack — comfortable for the current 5-stage +
async-regfile + async-dmem shape. Phase 2B / 2C will shorten the
X cone (M4K regfile, sync dmem + D\$) and lift the ceiling past
60 MHz.
...
-}
topEntity ::
  {- | 50 MHz clock generated by an Altera ALTPLL sitting in the
